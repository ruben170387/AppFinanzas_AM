...
# --- CONEXIÓN NUCLEAR (USANDO JSON PURO) ---
@st.cache_resource
def conectar_excel():
    try:
...
gastos_var = num(df_mov['Importe']).sum() if not df_mov.empty else 0
# Buscamos el % de ahorro si existe
...
disponible = ingresos - fijos - ahorro_obj - gastos_var
hoy = datetime.now()
...
c2.metric("Límite Hoy", f"{diario:.2f} €", help=f"Quedan {dias_restantes} días")

# Gráfico rápido
st.markdown("### 📊 Estado Actual")
df_graf = pd.DataFrame({"Tipo": ["Fijos", "Variables", "Ahorro", "Disponible"], 
                        "Euros": [fijos, gastos_var, ahorro_obj, max(0, disponible)]})
